...
impl Deck {
    ...
    // SOLI PATCH: mid-game constructor for the Soli app (hints/unwinnable solver).
    // `cards` is the current physical layout [waste bottom→top, then stock
    // next-draw-first→last] and `draw_cur` = number of waste cards. Mid-game we
    // don't know the original 24-card order, so we declare the current order
    // canonical: `map` gets each card's current index and `mask` is all-ones over
    // `cards.len()` bits. This is sound because `find_card`/`encode`/`decode` only
    // depend on the *relative* order of the remaining cards, which play
    // (`pop_next`/`push` at `draw_cur`) preserves; deal/iteration semantics only
    // read `deck`/`draw_cur`/`draw_step`. Verified against a played-through deck by
    // the draw-cycle equivalence tests in soli-solver-ffi.
    #[must_use]
    pub fn from_remaining(cards: &[Card], draw_cur: u8, draw_step: NonZeroU8) -> Self {
        debug_assert!(cards.len() <= N_DECK_CARDS as usize);
        debug_assert!(usize::from(draw_cur) <= cards.len());
