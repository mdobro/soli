...
// SOLI PATCH: added Clone (all fields are Clone) — the FFI crate clones the
// game to replay the winning line through convert_moves for hint derivation.
#[derive(Debug, Clone)]
...
impl StandardSolitaire {
    ...
    // Ordering conventions (verified by soli-solver-ffi equivalence tests):
    // - `hidden_piles[i]`: face-down cards bottom→top — the LAST element is
    //   revealed next (`do_move` reveals via `.pop()`). Column i holds at most i
    //   face-down cards (capacity invariant of `Hidden::from_piles`; callers
    //   must validate).
    // - `piles[i]`: face-up run bottom→top (`piles[i].last()` = top of the run).
    // - `deck_cards`/`draw_cur`: see `Deck::from_remaining`.
    // - `stack`: see `Stack::from_counts`.
    #[must_use]
    pub fn from_midgame(
        hidden_piles: [HiddenVec; N_PILES as usize],
        piles: [PileVec; N_PILES as usize],
        ...
        Self {
            hidden_piles,
            final_stack: stack,
            deck: Deck::from_remaining(deck_cards, draw_cur, draw_step),
            piles,
        }
    }
